...
class BankAccountController extends AppController
{
	...
	public function actionAdminDataJson()
	{
	    ...
	    $criteria = $this->getCriteriaFromRequest($bankAccount);
	    ...
	    $bankAccounts = $bankAccount->with(array(
			'glAccount' => array(
				'select' => 'fa_gl_account.name'
			),
			'bankAccountType' => array(
				'select' => 'fa_bank_account_type.name'
			),
			'currency' => array(
				'currency' => 'fa_currency.code'
			)
		))->findAll($criteria);
	    $count = $bankAccount->count();
	    
	    $this->renderPartial('adminDataJson',
			array(
